...
<div class="container-fluid">
    <div class="card">
        <div class="card-header">
            ...
            @endif
            <a href="{{ route('tambah-jadwal-kalibrasi') }}" class="btn btn-primary float-right"> Tambah Data</a>
        </div>
        <div class="card-body">
            <table class="table table-bordered" id="data-table-one">
                ...
                <tbody>
                    @php
                        $no = 1;
                    @endphp
                    @foreach ($jadwalKalibrasi as $item)
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td>{{ $item->title }}</td>
                            <td>{{ date('d F Y', strtotime($item->start))}}</td>
                            <td>{{ $item->end ? date('d F Y', strtotime($item->end)) : '-'}}</td>
                            <td>
                                <div class="flex align-items-center list-user-action">
                                    <a href="/edit-jadwal-kalibrasi/{{ $item->id }}"
                                        class="mr-1 btn btn-sm btn-icon btn-success" data-toggle="tooltip"
